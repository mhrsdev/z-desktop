...
/// jour-060: kind existence report — same single-line compact
/// `{kind, exists}` JSON as [`journal_kind_exists_jsonl`] under the
...
/// jour-062: thread count as a single-line compact JSONL string —
/// `{threads}` wrapping [`journal_thread_count`].
pub fn journal_thread_count_jsonl(path: &Path) -> Result<String, String> {
    let threads = journal_thread_count(path)?;
...
/// jour-066: [`journal_thread_names`] as compact JSONL — one thread id
/// string per line, ascending. An empty segment yields an empty string.
pub fn journal_thread_names_jsonl(path: &Path) -> Result<String, String> {
    let names = journal_thread_names(path)?;
    names
        .iter()
        .map(|n| serde_json::to_string(n).map_err(|e| e.to_string()))
        .collect::<Result<Vec<_>, _>>()
        .map(|lines| lines.join("\n"))
}

/// jour-067: [`journal_thread_names`] count as a single-line compact
/// `{threads}` JSON — same shape as [`journal_thread_count_report`], derived
/// from the sorted distinct name vec.
pub fn journal_thread_names_report(path: &Path) -> Result<String, String> {
    let threads = journal_thread_names(path)?.len();
    serde_json::to_string(&serde_json::json!({ "threads": threads })).map_err(|e| e.to_string())
}

/// jour-068: true if at least one thread exists in the segment —
/// `!journal_thread_names(path)?.is_empty()`. An empty segment is `false`.
pub fn journal_thread_names_exists(path: &Path) -> Result<bool, String> {
    Ok(!journal_thread_names(path)?.is_empty())
}

/// jour-069: thread-name existence as a single-line compact JSONL string —
/// `{threads}` wrapping [`journal_thread_names_exists`].
pub fn journal_thread_names_exists_jsonl(path: &Path) -> Result<String, String> {
...
/// jour-055: journal health as a single compact JSON line — `{records, bytes,
/// gaps, last_seq}` combining [`lag_stats`] and on-disk size (same basis as
/// [`journal_health_jsonl`], minus `pct_of_cap` plus `last_seq`). `last_seq`
/// is `null` on an empty segment.
pub fn journal_health_report(path: &Path) -> Result<String, String> {
    let stats = lag_stats(&Journal::replay(path)?);
    let bytes = std::fs::metadata(path).map_err(|e| e.to_string())?.len();
    serde_json::to_string(&serde_json::json!({
        "records": stats.records,
...
pub(crate) mod reducer_tests {
    ...
    #[test]
    fn journal_health_report_seeded_is_exact_line() {
        let dir = temp_dir("jour-055");
        {
            let mut j = Journal::open(&dir, "main").expect("open");
            append(&mut j, JournalKind::TurnStarted, Some("t1"), json!({}));
            append(&mut j, JournalKind::MessagePersisted, Some("t1"), json!({}));
        ...
        // json! uses a BTreeMap → keys serialize in sorted order.
        assert_eq!(
            journal_health_report(&path).expect("report"),
            format!("{{\"bytes\":{bytes},\"gaps\":0,\"last_seq\":3,\"records\":3}}")
        );
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn journal_health_report_missing_file_is_err() {
        let dir = temp_dir("jour-055-missing");
        assert!(journal_health_report(&dir.join("nope.jsonl")).is_err());
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    #[test]
    fn journal_kind_exists_report_seeded_true_unknown_false() {
        let dir = temp_dir("jour-060");
        {
            let mut j = Journal::open(&dir, "main").expect("open");
            append(&mut j, JournalKind::TurnStarted, Some("t1"), json!({}));
        }
        assert_eq!(
            journal_kind_exists_report(&dir.join("main.jsonl"), "turn_started").expect("report"),
            r#"{"exists":true,"kind":"turn_started"}"#
        );
        assert_eq!(
    ...
    #[test]
    fn journal_thread_count_jsonl_seeded_matches_exact_line() {
        let dir = temp_dir("jour-062");
        {
            let mut j = Journal::open(&dir, "main").expect("open");
            append(&mut j, JournalKind::TurnStarted, Some("t1"), json!({}));
            append(&mut j, JournalKind::MessagePersisted, Some("t2"), json!({}));
    ...
    #[test]
    fn journal_thread_count_jsonl_empty_segment_is_zero() {
        let dir = temp_dir("jour-062-empty");
        let _ = Journal::open(&dir, "main").expect("open");
        assert_eq!(
            journal_thread_count_jsonl(&dir.join("main.jsonl")).expect("jsonl"),
            r#"{"threads":0}"#
    ...
    #[test]
    fn journal_thread_names_jsonl_seeded_is_one_valid_line_per_name() {
        let dir = temp_dir("jour-066");
        {
            let mut j = Journal::open(&dir, "main").expect("open");
            append(&mut j, JournalKind::TurnStarted, Some("t2"), json!({}));
            append(&mut j, JournalKind::MessagePersisted, Some("t1"), json!({}));
        ...
        let jsonl = journal_thread_names_jsonl(&dir.join("main.jsonl")).expect("jsonl");
        let lines: Vec<&str> = jsonl.lines().collect();
        assert_eq!(lines, vec![r#""t1""#, r#""t2""#]);
        for line in &lines {
            serde_json::from_str::<String>(line).expect("valid json line");
        }
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn journal_thread_names_jsonl_empty_segment_is_empty_string() {
        let dir = temp_dir("jour-066-empty");
        let _ = Journal::open(&dir, "main").expect("open");
        assert_eq!(
            journal_thread_names_jsonl(&dir.join("main.jsonl")).expect("jsonl"),
            ""
        );
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    #[test]
    fn journal_thread_names_report_seeded_matches_exact_line() {
        let dir = temp_dir("jour-067");
        {
            let mut j = Journal::open(&dir, "main").expect("open");
            append(&mut j, JournalKind::TurnStarted, Some("t2"), json!({}));
            append(&mut j, JournalKind::MessagePersisted, Some("t1"), json!({}));
        ...
        assert_eq!(
            journal_thread_names_report(&dir.join("main.jsonl")).expect("report"),
            r#"{"threads":2}"#
        );
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    #[test]
    fn journal_thread_names_report_empty_segment_is_zero() {
        let dir = temp_dir("jour-067-empty");
        let _ = Journal::open(&dir, "main").expect("open");
        assert_eq!(
            journal_thread_names_report(&dir.join("main.jsonl")).expect("report"),
            r#"{"threads":0}"#
    ...
    #[test]
    fn journal_thread_names_exists_seeded_is_true() {
        let dir = temp_dir("jour-068");
        {
            let mut j = Journal::open(&dir, "main").expect("open");
            append(&mut j, JournalKind::TurnStarted, Some("t1"), json!({}));
        }
        assert!(journal_thread_names_exists(&dir.join("main.jsonl")).expect("exists"));
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn journal_thread_names_exists_empty_segment_is_false() {
        let dir = temp_dir("jour-068-empty");
        let _ = Journal::open(&dir, "main").expect("open");
        assert!(!journal_thread_names_exists(&dir.join("main.jsonl")).expect("exists"));
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    #[test]
    fn journal_thread_names_exists_jsonl_seeded_is_true_line() {
        let dir = temp_dir("jour-069");
        {
            let mut j = Journal::open(&dir, "main").expect("open");
            append(&mut j, JournalKind::TurnStarted, Some("t1"), json!({}));
        }
    ...
    #[test]
    fn journal_thread_names_exists_jsonl_empty_segment_is_false_line() {
        let dir = temp_dir("jour-069-empty");
        let _ = Journal::open(&dir, "main").expect("open");
        assert_eq!(
            journal_thread_names_exists_jsonl(&dir.join("main.jsonl")).expect("jsonl"),
            r#"{"threads":false}"#
